package com.ibm.activity.currencyconversionfactor.exception;
...
@ControllerAdvice
public class CurrencyCoExceptionHandler {
	...
	@ResponseStatus(value = HttpStatus.EXPECTATION_FAILED)
	public ResponseEntity<CoreError> handleException(ResourceUpdateException updateException)  {
		CoreError coreError= new CoreError();
		coreError.setAppliedValue(updateException.getMessage());
		coreError.setRejectedValue(updateException.getMessage());
		coreError.setErrorCode(HttpStatus.EXPECTATION_FAILED+"");
		coreError.setErrorMessage("Transactional Error");
		return new ResponseEntity<CoreError>(coreError, HttpStatus.EXPECTATION_FAILED);
	}
